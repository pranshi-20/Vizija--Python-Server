...
path="C:/Users/Jaadu/Desktop/daltonize/static/"
app=Flask(__name__)
...
def home():
    if api_call()==True:
        ori=Image.open(args.input_image)
        if args.simulate:
            simul_rgb = simulate(ori, args.type)
            simul_img = array_to_img(simul_rgb)
            simul_img.save(args.output_image)
            encoded=get_response_image(args.output_image)
            return jsonify(encoded)
        if args.daltonize:
            dalton_rgb = daltonize(ori, args.type)
